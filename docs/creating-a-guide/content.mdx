...
`setContent()` takes 2 arguments: content (interface Content) and defaults (interface ContentDefaults).
Data inside defaults and content objects is merged to create content object internally, more in `getContent()`.
...
Setting `closeOnLastNext` to `true` will close the guide when the last step is finished, IE, `Lusift.next()` is ran
...
if there is one. In absense of an active guide, this method will run `Lusift.showContent()` with id of the first enabled
guide in the content.

Run this on page changes in single page applications and when content, potentially incuding a target element, is dynamically added to the page.
## getActiveGuide
`Lusift.getActiveGuide()` returns the `id` and the `instance` object of the active guide if any. In absense of an active
