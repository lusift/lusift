      ...
      <Container>
        <label htmlFor="dropdown-input" className={cn('w-full', { opened })}>
          <input
            ...
            onChange={toggleOpen}
          />
          <div className="docs-select flex h-10 w-full leading-10 text-left pointer items-center">
            <ArrowRightSidebar className="text-gray-600 -ml-1" />
            Menu
          </div>
        </label>
        <div className="px-4 docs-dropdown shadow-xl" ref={menuRef}>
          <Container>
            <nav className="py-2.5">{children}</nav>
          </Container>
        </div>
        <style jsx>{`
          .docs-dropdown {
