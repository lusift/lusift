...
const renderProgressBar = () => {
  ...
  const currentStep = currentGuide.steps[
    currentGuide.trackingState.currentStepIndex
  ];
  ...
  if (!borderRadius){
    if (currentStep.type === 'modal'){
      if (currentStep.styleProps){
        borderRadius = currentStep.styleProps.borderRadius ||
          DEFAULT_MODAL_BORDER_RADIUS;
      } else {
  ...
  const progress = ((syncSteps.findIndex(step => {
    return step.index === currentStep.index
  })
  + 1) / (syncSteps.length+1))*100;
